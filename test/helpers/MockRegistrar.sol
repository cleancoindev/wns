...
contract MockRegistrar {
    ...
    uint32 constant totalAuctionLength = 5 * 60;//5 days;
    uint32 constant revealPeriod = 2  * 60;//48 hours;
    uint32 public constant launchLength = 100 ;//weeks;
     ...
     */
    function MockRegistrar(WNS _wns, bytes32 _rootNode, uint _startDate) public {
        wns = _wns;
        rootNode = _rootNode;
    ...
    function newBid(bytes32 sealedBid) public payable {
        ...
        Deed n = (new Deed).value(msg.value)(msg.sender);
        sealedBids[msg.sender][sealedBid] = n;
    ...
    function transferRegistrars(bytes32 _hash) public onlyOwner(_hash) {
        ...
        MockRegistrar(registrar).acceptRegistrarTransfer(_hash, h.deed, h.registrationDate);
     ...
     */
    function acceptRegistrarTransfer(bytes32 hash, Deed deed, uint registrationDate) public pure {
